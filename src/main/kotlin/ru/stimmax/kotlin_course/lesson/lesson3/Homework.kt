...
// 41. Контактная информация экстренных служб, медицинского персонала
// и других важных служб, для внутреннего пользования.
// (приватное иницализация сразу неизменяемое)
private val emergencyContacts:String = " xnjwkncowk"
// 42. Особые условия для участников с ограниченными возможностями
//(публичное инициализация по запросу не изменяемое)
val accessibilityConditions:String by lazy {
    "инфа"
}
// 43.Общее настроение участников (определяется опросами)
// (изменяемое приватное инициализация позже)
private lateinit var participantsMood:String
...
// (не изменяемое приватное инциализация ленивая)
private val detailedHackathonPlan:String by lazy {
    "Plan"
}
// 45. Имя знаменитого специального гостя,
// которое будет объявлено за день до мероприятия,
// но в момент инициализации неизвестно.
// (приватное инициализация отложена измениемое)
private lateinit var specialGuestName:String
// 46. Максимальное количество людей, которое может вместить место проведения.
// (приватное не изменяемое известно заранее)
private val maximumAttendees:Int by lazy {
    300
}
